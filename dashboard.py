...
# Função principal da página 1
def pagina_um():
    st.title("Análise de Dados de Energia")

    # Carregar os dados e calcular correlações
    data, correlation_matrix = load_data()
    ...
    if selected_variable:
        ...
        cor_color = "#398e3d" if correlation_value >= 0.0 else "#fb4d4c"
        formatted_correlation = "{:.8f}".format(correlation_value)

        st.metric(label="Correlação", value= formatted_correlation, delta=0, delta_color="inverse")
